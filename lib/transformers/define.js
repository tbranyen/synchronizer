...
NormalizeDefineTransformer.prototype.singleArgument = function() {
  ...
  var normalized = this.context.tree.moduleToPath(this.context.fileName, this.context.tree.baseDir).slice(0, -3);

  console.log('single define', normalized);

  arg.update("\"" + normalized + "\", " + arg.source());
};
  ...
  if (arg.type !== "Literal" && !utils.isString(arg)) {
    ...
    if (arg.type === "ArrayExpression") {
      previousValue = "[" + arg.elements.map(function(element) {
        ...
        console.log(element.value, normalized);
        return "\"" + normalized + "\"";
      }, this) + "]";
    }

    console.log('multi define', moduleName);
