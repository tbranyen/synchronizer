...
var onBuildStart = _.once(function(config, tree) {
  ...
  Object.keys(config.paths || {}).forEach(function(key) {
    tree.paths[key] = path.normalize(config.paths[key], tree.baseDir);
  });
});
...
exports.optimize = function(fileName, config) {
  ...
  config.moduleName = tree.pathToModule(fileName);

  // Set the context on the tree.
  tree.context = config.context;

  // This runs once.
  onBuildStart(config, tree);
